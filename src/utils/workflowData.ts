...
import { 
  ...
  matrixMultiply, matrixAdd, relu, softmax, crossEntropyLoss, 
  matrixSubtract, transpose, relu_derivative, elementWiseMultiply
...
const A3 = softmax(Z3);
const LOSS = crossEntropyLoss(A3, Y_TRUE);

// --- BACKPROPAGATION CALCULATIONS ---
const dZ3 = matrixSubtract(A3, Y_TRUE);
const dW3 = matrixMultiply(transpose(A2), dZ3);
const db3 = dZ3;
const dA2 = matrixMultiply(dZ3, transpose(NN_W3));
const dZ2 = elementWiseMultiply(dA2, relu_derivative(Z2));
const dW2 = matrixMultiply(transpose(A1), dZ2);
const db2 = dZ2;
const dA1 = matrixMultiply(dZ2, transpose(NN_W2));
const dZ1 = elementWiseMultiply(dA1, relu_derivative(Z1));
const dW1 = matrixMultiply(transpose(NN_INPUT), dZ1);
const db1 = dZ1;
